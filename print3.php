...
if (!in_array(strtolower($fileExtension), $allowedExtensions)) {
    ...
    // 执行打印操作
    exec('lp -o fit-to-page -o media=A4 -o sides=two-sided-long-edge ' . escapeshellarg($filePath), $output, $return_var); // 使用 escapeshellarg 函数确保文件路径中的特殊字符被正确处理
    ...
    if ($return_var === 0) {
        ...
        if ($fileExtension === 'pdf') {
            ...
            echo "打印费用为：" . ($pageCount * 0.075) . "元。"; // 将页数乘以 0.075，显示打印费用
